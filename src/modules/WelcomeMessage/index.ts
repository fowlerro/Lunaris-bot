...
class WelcomeMessageModule extends BaseModule {
	...
	async add(guildId: Snowflake, format: WelcomeMessageFormat): Promise<WelcomeMessage | null> {
		...
		const config = await WelcomeMessageModel.findOneAndUpdate(
			{ guildId },
			{
				$push: { formats: format },
			},
	...
	async delete(guildId: Snowflake, format: WelcomeMessageFormat): Promise<WelcomeMessage | null> {
		...
		const config = await WelcomeMessageModel.findOneAndUpdate(
			{ guildId, 'formats.message': format.message, 'formats.action': format.action },
			{
				$pull: {
					formats: {
	...
	async setCache(document: HydratedDocument<WelcomeMessage>) {
	...
	async list(
		guildId: Snowflake,
		action?: WelcomeMessageAction | null
	): Promise<WelcomeMessageFormat[] | GroupedWelcomeMessageFormats | null> {
		if (!guildId) return null;
