...
export default async (interaction: CommandInteraction) => {
	...
	const config = await Logs.set(interaction.guildId!, category, channel?.id);
	if (!config) return handleCommandError(interaction, 'general.error');

	const embed = new MessageEmbed().setColor(palette.success).setDescription(t('command.logs.set', language));
